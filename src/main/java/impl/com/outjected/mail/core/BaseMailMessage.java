...
public class BaseMailMessage implements MailMessage {
	...
	public void addRecipient(RecipientType recipientType,
			EmailContact emailContact) throws SeamMailException {
		try {
			rootMimeMessage.addRecipient(recipientType.getRecipientType(),
					MailUtility.getInternetAddress(emailContact));
		} catch (MessagingException e) {
			throw new SeamMailException("Unable to add recipient "
					+ recipientType + ": " + emailContact.toString()
					+ " to MIME message", e);
		}
	}

	public void addRecipients(RecipientType recipientType,
			EmailContact[] emailContacts) throws SeamMailException {
		try {
			rootMimeMessage.addRecipients(recipientType.getRecipientType(),
					MailUtility.getInternetAddressses(emailContacts));
		} catch (MessagingException e) {
			throw new SeamMailException("Unable to add " + recipientType
					+ ":  Collection<Recipients>to MIME message", e);
		}
	}

	public void addRecipients(RecipientType recipientType,
			Collection<EmailContact> emailContacts) throws SeamMailException {
		try {
			rootMimeMessage.addRecipients(recipientType.getRecipientType(),
					MailUtility.getInternetAddressses(emailContacts));
		} catch (MessagingException e) {
	...
	public void subject(String subject, String charset)
		...
		try {
			...
			throw new SeamMailException("Unable to add subject:" + subject
	...
	public void setFrom(EmailContact emailContact) throws SeamMailException {
		try {
			rootMimeMessage.setFrom(MailUtility
					.getInternetAddress(emailContact));
		} catch (MessagingException e) {
			throw new SeamMailException("Unable to add From Address:"
					+ emailContact.getEmailAddress()
					+ " to MIME message with charset: "
					+ emailContact.getCharset(), e);
		}
	}
	...
	public void setSentDate(Date date) throws SeamMailException {
		try {
			...
			throw new SeamMailException(
					"Unable to set Sent Date on MimeMessage", e);
		}
	}
	...
	public void importance(MessagePriority messagePriority)
			throws SeamMailException {
	...
	public void setHeader(String name, String value) throws SeamMailException {
		try {
			...
			throw new SeamMailException("Unable to SET Header: + " + name
					+ " to Value: " + value, e);
		}
	}
	...
	public void addHeader(String name, String value) throws SeamMailException {
		try {
			...
			throw new SeamMailException("Unable to ADD Header: + " + name
					+ " to Value: " + value, e);
		}
	}
	...
	public void setTextBody(String text) throws SeamMailException {
		try {
			...
			throw new SeamMailException(
					"Unable to add TextBody to MimeMessage", e);
		}
	}
	...
	public void setHTMLBodyTextAlt(String html, String text)
			throws SeamMailException {
	...
	private MimeBodyPart buildTextBodyPart(String text)
		...
		try {
			textBodyPart
					.setDisposition(ContentDisposition.INLINE.headerValue());
	...
	public void addAttachment(File file, ContentDisposition contentDisposition)
			throws SeamMailException {
		Attachment attachment = new Attachment(file, file.getName(),
				contentDisposition);
		addAttachment(attachment);
	}

	public void addAttachment(File file, String fileName,
			ContentDisposition contentDisposition) throws SeamMailException {
		Attachment attachment = new Attachment(file, fileName,
				contentDisposition);
		addAttachment(attachment);
	}

	public void addAttachment(byte[] bytes, String fileName, String mimeType,
			ContentDisposition contentDisposition) throws SeamMailException {
		Attachment attachment = new Attachment(bytes, fileName, mimeType,
				contentDisposition);
		addAttachment(attachment);
	}
	...
	public void addAttachment(String url, String fileName,
		...
		try {
			...
			throw new SeamMailException("Unable to add attachment with URL: "
					+ url, e);
		}

		Attachment attachment = new Attachment(new URLDataSource(u), fileName,
				contentDisposition);
		addAttachment(attachment);
	}
		...
		for (Attachment a : attachments.values()) {
			if (a.getContentDisposition() == ContentDisposition.ATTACHMENT) {
				try {
					...
					throw new SeamMailException(
							"Unable to Add STANDARD Attachment: "
				...
				try {
					...
					throw new SeamMailException(
							"Unable to Add INLINE Attachment: "
									+ a.getAttachmentFileName(), e);
				}
			} else {
				throw new SeamMailException(
						"Unsupported Attachment Content Disposition");
			}
		}
	}
